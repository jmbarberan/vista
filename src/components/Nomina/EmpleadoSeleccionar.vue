<template>
  <b-modal class="modal-dialog-sel"
    v-model="verModal" title="Seleccionar cliente" 
  >
...
export default {
  ...
  methods: {
      ...
      switch (ev.keyCode) {
        case 13: {
          if (this.seleccionado != null)
            this.$emit("buscadorCli-seleccionado", this.seleccionado);
          break;
        }
        case 32: {
    ...
    buscar() {
      ...
      this.$store
        .dispatch("maestros/clientesPorNombre", p)
        .then(function(r) {
          if (r.data != undefined) {
              this.selIndex = -1;
              this.items = r.data;
    ...
    cargarBusqueda(nom, avz) {
      if (nom.length > 0) {
        this.busquedaTexto = nom;
